...
import { CalendarDays, Eye, FilePenLine, MapPin, Radio, Trash2 } from "lucide-react";
...
export function MatchCard({
  ...
  const canViewResult = match.status === "Completed" && Boolean(match.tournamentId && match.matchNumber);
  const resultHref = canEditExistingResult
    ...
    : "/results";
  const viewHref = canViewResult
    ? `/results/view?tournament=${encodeURIComponent(match.tournamentId ?? "")}&match=${encodeURIComponent(String(match.matchNumber ?? ""))}`
    : null;
    ...
    <article className="panel overflow-hidden rounded-xl p-3 sm:p-4">
      ...
      <div className="mt-3 grid gap-2 sm:flex sm:flex-wrap">
        {viewHref ? (
          <Link
            href={viewHref}
            className="inline-flex items-center justify-center gap-2 rounded-lg border border-[#00D4FF]/25 bg-[#00D4FF]/10 px-3 py-2 text-xs font-semibold text-[#8BE8FF] sm:justify-start"
          >
